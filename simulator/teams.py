...
class Team:
    attlist = ['long_name', 'nationality', 'overall', 'player_positions', 'team_position',
    ...
    def __init__(self, team_name):
        ...
        self.setManager()
        self.playingSquad()
        self.attack = round(np.array([a.stats['overall'] for a in self.players.values() if a.stats['player_positions'] == 'Attacker' and (
            a.stats['team_position'] == 'Starter' or a.stats['team_position'] == 'Substitute')]).mean())
        self.midfield = round(np.array([a.stats['overall'] for a in self.players.values() if a.stats['player_positions'] == 'Midfielder' and (
            a.stats['team_position'] == 'Starter' or a.stats['team_position'] == 'Substitute')]).mean())
        self.defence = round(np.array([a.stats['overall'] for a in self.players.values() if (a.stats['player_positions'] == 'Defender' or a.stats['player_positions'] == 'GK') and (
            a.stats['team_position'] == 'Starter' or a.stats['team_position'] == 'Substitute')]).mean())

    def addPlayers(self):
        df = pd.read_pickle('player_data')
    ...
    def showPlayers(self, player_dict=None):
        ...
        for player in player_dict.values():
            ...
            df = df.append(temp)
            i += 1
        print(tabulate(df, headers=[c.capitalize()
              for c in df.columns], tablefmt='psql', stralign='left'))
    ...
    def showManager(self):
        table_data = []
        table_data.append([self.manager.stats['long_name'], 'Manager',
                          self.manager.stats, self.manager.ivs, self.manager.formation])
    ...
    def playingSquad(self):
        ...
        squad.update({'attackers': list(squad_attackers.values()), 'midfielders': list(squad_midfielders.values(
        )), 'defenders': list(squad_defenders.values()), 'goalkeeper': list(squad_gk.values())})
        self.squad = squad
    ...
    def Substitutes(self):
        ...
        reserves = [r for r in self.players.items() if (
            r not in self.squad.items() and r not in self.subs.items())]
