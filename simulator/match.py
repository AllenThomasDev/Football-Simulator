...
class Match:
    ...
    foulkeys = ['Free kick won', 'Yellow card',
    ...
    def __init__(self, home_side, away_side):
        ...
        tlist.extend(['On target', 'Saved', 'Off target',
                     'Blocked', 'Hit the bar', 'Goal'])
        ...
        self.setOdds()
        self.matchevents = []
        self.stats = dict(zip([self.home_side, self.away_side], [
        ...
        for minute in range(100):
            for _ in range(135):
                if(random.uniform(0, 1) < self.odds[minute]['Event']):
                    ...
                    side = random.choices(
                        [self.home_side, self.away_side], plist)[0]
                    event = random.choices(Match.eventkeys, list(
                        self.odds[minute][self.sides[side]]['Events'].values()))[0]
    ...
    def showStats(self):
        ...
        print(tabulate(table, headers=[
              '', self.home_side.team_name, self.away_side.team_name]))
